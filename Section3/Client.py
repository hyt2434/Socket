...
class Client:
    ...
    def __init__(self, master, serveraddr, serverport, rtpport, filename):
        ...
        self.PRE_BUFFER_SIZE = 40  # Number of frames to pre-buffer
    ...
    def createWidgets(self):
        ...
        Label(self.statsFrame, text="Network Statistics", font=("Arial", 10)).grid(
        ...
        # BUFFER STATUS PANEL 
        # ---------------------------------------------------------
        self.bufferFrame = LabelFrame(self.master, text="Buffer Status", font=("Arial", 10), padx=5, pady=5)
        self.bufferFrame.grid(row=3, column=0, columnspan=4, sticky="we", padx=10, pady=5)

        # Progress Bar
        self.bufferBar = ttk.Progressbar(self.bufferFrame, orient="horizontal", mode="determinate", length=600)
        self.bufferBar.pack(fill="x", padx=5, pady=5)
        self.bufferBar["maximum"] = 100 

        
        self.bufferLabel = Label(self.bufferFrame, text="0/100 frames (0.0%)", fg="blue", font=("Arial", 10))
        self.bufferLabel.pack(anchor="e", padx=5)
            ...
            try:
                ...
                if data:
                    ...
                    if rtp.getMarker() == 1:
                        ...
                        print(f"Frame {self.recv_frame_count}: {framesize} bytes needs {packet_count} packets")

                        
                        if not self.frame_buffer.full():
                            self.frame_buffer.put(bytes(temp_buf))
                        else:
    ...
    def run_buffer_processing(self):
        """ Process frames from buffer and update GUI"""
        if self.state == self.PLAYING:
            

            current_size = self.frame_buffer.qsize()
            percentage = (current_size / 100) * 100 
            
            # --- LOGIC PRE-BUFFERING ---
            if self.is_buffering:
                self.bufferLabel.config(text=f"Buffering... {current_size}/{self.PRE_BUFFER_SIZE} frames")
                self.bufferBar["value"] = current_size
                if current_size < self.PRE_BUFFER_SIZE:
                    if self.state != self.TEARDOWN:
                        self.master.after(10, self.run_buffer_processing)
                    return
                else:
                    self.is_buffering = False
                    print("Pre-buffering complete! Starting playback.")

                    if self.is_first_play:
                        self.startTime = time.time() # Reset start time for stats
                        self.totalBytes = 0          # Reset stats
                        self.totalPackets = 0        # Reset packet count
                        self.lossPackets = 0         # Reset lost packets
                        self.frameNbr = 0            # Reset displayed frames
                        self.jitter = 0.0            # Reset jitter
            ...
            self.bufferBar["value"] = current_size
            self.bufferLabel.config(text=f"{current_size}/100 frames ({percentage:.1f}%)")
            # ---------------------------------------
        ...
        if self.state != self.TEARDOWN:
            self.master.after(40, self.run_buffer_processing)
    # ---------------------------------------------------------
    # RTSP
    # ---------------------------------------------------------
            ...
            if self.sessionId == session:
                ...
                if code == 200:
                    if self.requestSent == self.SETUP:
                        ...
                        print("PLAY confirmed, buffering started...")
